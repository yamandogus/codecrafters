...
export class AuthService {
    ...
    async register(data) {
        ...
        if (existing) {
            const e = new Error("Bu e-posta ile zaten kayıtlı bir kullanıcı var");
            e.status = 400;
            throw e;
        }
    ...
    async login(data) {
        ...
        if (!user) {
            const e = new Error("Geçersiz e-posta veya şifre");
            e.status = 401;
            throw e;
        }
    ...
    async refresh(token) {
        try {
            ...
            if (!stored || !stored.user) {
                const e = new Error("Geçersiz veya süresi dolmuş yenileme tokenı");
                e.status = 401;
                throw e;
            }
            ...
            const e = new Error("Yenileme başarısız");
            e.status = 401;
            throw e;
        }
    }
